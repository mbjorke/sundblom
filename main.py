...
def _log_tokens(input_tokens: int, output_tokens: int) -> None:
    ...
    body: dict = {
        "message": f"📊 Token-logg: {input_tokens}+{output_tokens} tokens [skip ci]",
...
def save_seen_url(url: str, headline: str, date_iso: str) -> None:
    ...
    payload: dict = {
        "message": f"🔖 Markerar som processad: {headline[:50]} [skip ci]",
...
def save_article_json(headline: str, julius_text: str, body: str, author: str,
    ...
    _push_file(path, content, f"🗞️ Ny artikel: {headline[:60]} [skip ci]")
